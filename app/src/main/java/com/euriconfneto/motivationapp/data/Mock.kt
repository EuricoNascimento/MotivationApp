package com.euriconfneto.motivationapp.data
...
class Mock {
    ...
    fun getPhrase(id: Int): String{
        val filter = mListPhrases.filter { it.categoryId == id || id == all }
        val rando = Random.nextInt(filter.size)
        return filter[rando].description
    }
}